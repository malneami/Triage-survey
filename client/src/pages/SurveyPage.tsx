      ...
      {q.type === "checkbox" && q.options && (
        <div className={`grid gap-2 ${q.twoCol ? "grid-cols-2" : "grid-cols-1"}`}>
          {q.options.map((opt) => {
            ...
            const toggle = () => {
              if (!checked && q.maxSelect && selected.length >= q.maxSelect) return; // cap reached
              const next = checked
            ...
            return (
              ...
              >
                <input
                  type="checkbox" name={q.id} value={opt.value} checked={checked}
                  onChange={toggle}
...
export default function SurveyPage() {
  ...
  function validate(): boolean {
    ...
    for (const rule of rules) {
      ...
      if (rule.type === "role") {
        valid = part !== null;
      } else if (rule.type === "radio" || rule.type === "scale" || rule.type === "checkbox") {
  ...
  function handleSubmit() {
    if (submitted) return; // guard against double-click double-submission
    if (!validate()) return;
    // Timestamp in Saudi local time (Asia/Riyadh), e.g. "2026-07-11 15:30:22"
    const riyadhTime = new Intl.DateTimeFormat("en-CA", {
